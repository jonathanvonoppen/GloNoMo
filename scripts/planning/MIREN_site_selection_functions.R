...
# Fieldwork preparation: site selection -- custom functions

# script by Jonathan von Oppen  //  jonathan.vonoppen@unibas.ch
# last updated 05 Mar 2026


# Download DEM along target_roads ----
  #> Reads download links from a csv file to download, mosaic and save DEM along the road
  #> returns a character string reporting success/status of the operation 
download_road_dem <- function(target_road,  # target road string matching the download links CSV file
                                              # and the string supplied to getMIRENsites() later on
                              dem_dir = input_data_dir  # directory containing the download links file
                              ){
  
  download_links_file <- list.files(dem_dir, pattern = paste0(target_road, "_road_ch.swisstopo.swissalti3d.*.csv")
                                    , full.names = T)
  if(length(download_links_file == 0)) stop(paste0("No csv file with download links was found that matches the target_road pattern.\n
                                                    Double-check at ", dem_dir))
  if(length(download_links_file > 1)) stop(paste0("More than one csv file with download links was found that matches the target_road pattern.\n
                                                    Double-check at ", dem_dir))
  
  # load download links obtained from https://www.swisstopo.admin.ch/en/height-model-swissalti3d#swissALTI3D---Download
  download_links <- readr::read_lines(download_links_file)
...
create_perpendicular_plots <- function(line_geom, 
  ...
  left_line <- sf::st_linestring(matrix(
    c(point_coord[1], point_coord[2], left_end[1], left_end[2]),
    ncol = 2, byrow = TRUE
  ))
  left_plot <- left_line %>% 
  ...
  right_end <- c(point_coord[1] - perp_dx * length, 
                 point_coord[2] - perp_dy * length)
  right_line <- sf::st_linestring(matrix(
    c(point_coord[1], point_coord[2], right_end[1], right_end[2]),
    ncol = 2, byrow = TRUE
  ))
...
# Filter perpendicular plot candidates ----
  #> applies different spatial filters to potential plots based on OSM data
...
## ... for NAs in raster (e.g. masked DEM) ----
  #> returns a logical vector of length nrow(polygon)
  #> TRUE if cell/polygon contains NAs
...
## ... for intersections with road or other line geometries ----
  #> returns TRUE if plot area intersects with road, FALSE if not
  #> to be used with dplyr::mutate()
...
## ... for intersections with other polygon geometries, e.g. lakes, buildings ----
  #> returns TRUE if plot area intersects with road, FALSE if not
  #> to be used with dplyr::mutate()
...
## Determine length of longest left-right alternating plot sequence ----
...
# Get points at roadside ----
  #> Determine location of exact plot establishment point at the side of the roadside for a plot polygon
...
# Derive potential sampling zones/sites ----
  #> Main function, performing automated site selection
getMIRENsites <- function(target_road  # road/trail
  ...
  # get elevational distance between sites
  site_elev_mean_dist <- site_elev %>% 
    dplyr::pull(elevation_m) %>% 
    mean()
  
  ## ~~ initiate loop to extend site elevation threshold for sites w/o suitable plots ----
  
  # get 5-m steps to raise threshold distance from optimal elevation if necessary
  elev_dist_thresh_steps <- seq.int(from = 0, to = site_elev_mean_dist/2
                                    , by = 5)
  
  # initialise sites_missing and  object
  sites_missing <- NULL
  
  for(add_elev_dist in elev_dist_thresh_steps){
    ...
    road_vertices_zones_strict <- road_vertices %>% 
      dplyr::filter(
        purrr::map_lgl(elevation_m, ~ any(
          .x >= site_elev$elevation_m - site_elev_dist_threshold - add_elev_dist & 
            .x <= site_elev$elevation_m + site_elev_dist_threshold + add_elev_dist
        ))
      ) %>% 
    ...
    if(length(sites_missing) > 0){
      ## print warning
      message(paste0(stringr::str_to_title(target_road), ": after applying spatial filters with current settings, no available plots remain at site(s) ", paste(sites_missing, collapse = ", "), "."))
      message(paste0("Re-running filters for site(s) ", paste(sites_missing, collapse = ", "), " ..."))
  ...
  sites_lr_available <- perp_plots_bothsides_selection %>% 
    ...
    {if(length(sites_missing) > 0) dplyr::bind_rows(., sites_missing_df) else .} %>% 
